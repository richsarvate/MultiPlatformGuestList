...
def sort_show_dates_chronologically(show_dates: List[str], reverse: bool = True) -> List[str]:
    """
    Sort show dates chronologically
    
    Args:
        show_dates: List of show date strings like "Wednesday May 7th 8pm"
    ...
    """
    try:
        # Create list of (parsed_date, original_string) tuples
        date_tuples = []
        for date_str in show_dates:
            parsed_date = parse_show_date(date_str)
            if parsed_date:
                date_tuples.append((parsed_date, date_str))
            else:
                # Keep unparseable dates at the end
                date_tuples.append((datetime.min if reverse else datetime.max, date_str))
        
        # Sort by parsed date
        date_tuples.sort(key=lambda x: x[0], reverse=reverse)
        
        # Return just the original strings
        return [date_str for _, date_str in date_tuples]
        
    except Exception as e:
        logger.error(f"Error sorting show dates: {e}")
        return show_dates  # Return original order if sorting fails

def get_most_recent_show_date(show_dates: List[str]) -> Optional[str]:
