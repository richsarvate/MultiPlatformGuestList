...
# Configure logging
LOG_FILE = "/home/ec2-user/GuestListScripts/logs/bucketlist_sales.log"
try:
...
CONFIG_FILE = "/home/ec2-user/GuestListScripts/config/bucketlistConfig.json"
try:
    ...
    for exp_data in active_experiences:
        ...
        for event in events:
            ...
            if has_new_sales and (new_tickets > 0 or force_refresh):
                ...
                # Create price lookup table for this event
                price_lookup = {}
                for ticket_type in event["ticketTypes"]:
                    ticket_name = ticket_type["name"]
                    price_cents = ticket_type.get("basePriceInCents", 0)
                    price_dollars = price_cents / 100 if price_cents else 0
                    price_lookup[ticket_name] = price_dollars
                ...
                for guest in guests:
                    ...
                    # Handle pair tickets (double the quantity)
                    if "pair" in ticket_type.lower():
                        ticket_quantity *= 2

                    # Calculate total price
                    unit_price = price_lookup.get(ticket_type, 0)
                    # For pair tickets, price is for the pair, not per person
                    if "pair" in ticket_type.lower():
                        total_price = unit_price * guest["quantity"]  # Original quantity before doubling
                    else:
                        total_price = unit_price * ticket_quantity
                    
                    logger.debug(f"Guest {guest['customerName']}: {ticket_type} x{guest['quantity']} = ${total_price:.2f}")
