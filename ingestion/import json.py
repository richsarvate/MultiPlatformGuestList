...
from getBucketlistCookie import get_bucketlist_cookie, load_cookie, get_new_cookie
    ...
    for exp_id in active_experiences:
        ...
        for event in events:
            ...
            if has_new_sales and (new_tickets > 0 or force_refresh):
                ...
                if not guests:
                    continue
                    # Create price lookup table from ticket types
                price_lookup = {}
                for ticket_type in event.get("ticketTypes", []):
                    ticket_name = ticket_type["name"]
                    price_in_cents = ticket_type["basePriceInCents"]
                    price_in_dollars = price_in_cents / 100.0
                    price_lookup[ticket_name] = price_in_dollars
                logger.info(f"Price lookup for {event_name}: {price_lookup}")
                ...
                for guest in guests:
                    ...
                    # Calculate total price using the price lookup table
                    ticket_type = guest["ticketType"]
                    price_per_ticket = price_lookup.get(ticket_type, 0.0)
                    total_price = price_per_ticket * ticket_quantity
                    logger.debug(f"Price calculation: {ticket_type} × {ticket_quantity} = ${total_price:.2f}")
